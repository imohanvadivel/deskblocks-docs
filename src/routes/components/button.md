...
| `type`          | <code>'button' &#124; 'submit' &#124; 'reset'</code>                                | 'button'  | Sets the native button type. Defaults to `'button'` so a Button inside a form never submits it by accident; pass `'submit'` for a form-submit button. Ignored when `href` is set. |
